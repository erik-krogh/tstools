...
public class UnionConstraintVisitor implements ExpressionVisitor<UnionNode>, StatementTransverse<UnionNode> {
    ...
    public final class IncludesWithFieldsResolver extends UnionFindCallback {
        ...
        public void run() {
            ...
            for (Snap.Obj closure : functions) {
                switch (closure.function.type) {
                    case "user":
                    case "bind": {
                        if (typeAnalysis.getOptions().staticMethod == Options.StaticAnalysisMethod.UNIFICATION_CONTEXT_SENSITIVE && !hasAnalysed.contains(closure)) {
