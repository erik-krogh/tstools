        ...
        public void run() {
            ...
            for (Snap.Obj closure : functions) {
                switch (closure.function.type) {
                    case "user":
                    case "bind": {
                        if (typeAnalysis.options.staticMethod == Options.StaticAnalysisMethod.TRADITIONAL_UNIFICATION_RECURSIVELY_RESOLVE_CALLGRAPH && !hasAnalysed.contains(closure)) {
                        ...
                        solver.union(this.functionNode, UnionConstraintVisitor.this.functionNodes.get(closure));
